...
function calc_cart_price() {
    ...
    obj.children('tbody').each(function(){
        ...
        allTotal = allTotal + eachTotal + eachTotal_tax;
        $(this).children('tr').last().find('em[nc_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
    });
    $('#cartTotal0').html(number_format(allTotal,2));
	
    //每个店铺商品价格小计
    obj1 = $('table[nc_type="table_cart1"]');
    if(obj1.children('tbody').length==0) return;
    //购物车已选择商品的总价格
    var allTotal1 = 0;
    obj1.children('tbody').each(function(){
        //购物车每个店铺已选择商品的总价格
        var eachTotal = 0;
		var eachTotal_tax = 0;
        $(this).find('em[nc_type="eachGoodsTotal"]').each(function(){
        ...
        allTotal1 = allTotal1 + eachTotal + eachTotal_tax;
        $(this).children('tr').last().find('em[nc_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
    
	});
    $('#cartTotal1').html(number_format(allTotal1,2));
	//税金

 
		
	    //每个店铺商品价格小计
    obj2 = $('table[nc_type="table_cart2"]');
    if(obj2.children('tbody').length==0) return;
    //购物车已选择商品的总价格
    var allTotal2 = 0;
	var eachTotal_tax = 0;
    obj2.children('tbody').each(function(){
        //购物车每个店铺已选择商品的总价格
        var eachTotal = 0;
        $(this).find('em[nc_type="eachGoodsTotal"]').each(function(){
            if ($(this).parent().parent().find('input[type="checkbox"]').eq(0).attr('checked') != 'checked') return;
        ...
        allTotal2 = allTotal2 + eachTotal + eachTotal_tax;
        $(this).children('tr').last().find('em[nc_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
    });	
	$('#cartTotal2').html(number_format(allTotal2,2));
...
$(function(){
    ...
    $('#next_submit0').on('click',function(){
	 ...
	 $('#next_submit1').on('click',function(){
        if ($(document).find('input[nc_type="eachGoodsCheckBox"]:checked').size() == 0) {
            showDialog('请选中要结算的商品', 'eror','','','','','','','','',2);
            return false;
        }else {
	 ...
	 $('#next_submit2').on('click',function(){
        if ($(document).find('input[nc_type="eachGoodsCheckBox"]:checked').size() == 0) {
            showDialog('请选中要结算的商品', 'eror','','','','','','','','',2);
            return false;
        }else {
