...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?act=country_api&op=code_update',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommends_list":
			...
			break;
		case "brand_list":
			$("#show_brand_list").load('index.php?act=country_api&op='+id);//查询数据
...
function get_goods_class() {//查询子分类
	...
	if (gc_id>0) {
		$.get('index.php?act=country_api&op=category_list&id='+gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?act=country_api&op=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
function del_recommend(id) {//删除商品推荐
    if ($(".right-country dl").size()<2) {
         return;//保留一个
    }
	if(confirm('您确定要删除吗?')) {
		$(".right-country dl[recommend_id='"+id+"']").remove();
...
function add_recommend() {//增加商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".right-country dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
			...
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			...
    			'<dd><ul class="goods-list-country"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
			...
			$("#add_recommend_list").before('<dl select_recommend_id="'+i+'"><dt><h4 class="dialog-handle-title">商品推荐模块标题名称</h4>'+
    			...
    			'<span class="right-country">修改该区域中部推荐商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏</span>'+
    			'<div class="clear"></div></div></dt><dd><h4 class="dialog-handle-title">推荐商品</h4><ul class="dialog-goodslist-s1 goods-list-country">'+
...
function update_recommend() {//更新
	...
	if (get_text=='1') {
		...
		$(".right-country dl[recommend_id='"+id+"'] dt h4").html(recommend_name);
	  ...
	  $(".right-country dl[recommend_id='"+id+"'] dd").html('<ul class="goods-list-country">'+text_append+'</ul>');
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='') {
		$("#show_brand_list").load('index.php?act=country_api&op=brand_list&'+$.param({'brand_name':brand_name }));
	}
}
...
//中部推荐图片上传
function show_recomm2end_pic_dialog(id) {//弹出框
...
//切换图片上传
function add_slide_adv_disable() {//增加图片
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load('index.php?act=country_api&op=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));
	}
}
	...
	for (var i = 1; i <= sale_max; i++) {//防止数组下标重复
		if ($("dl[sale_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			$("#select_sale_list").before('<dl select_sale_id="'+i+'"><dt><h4 class="dialog-handle-title">商品推荐模块标题名称</h4>'+
    			...
    			'<div class="clear"></div></div></dt><dd><ul class="dialog-goodslist-s1 goods-list-country">'+
...
function get_recommends_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommends_goods_list").load('index.php?act=country_api&op=recommends_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
