...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?act=country_api&op=code_country_update',
...
function del_recommend(id) {//删除商品推荐
    if ($(".right-country-detail dl").size()<2) {
         return;//保留一个
    }
	if(confirm('您确定要删除吗?')) {
		$(".right-country-detail dl[recommend_id='"+id+"']").remove();
...
function add_recommend() {//增加商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".right-country-detail dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
			...
			$("#add_recommend_list").before('<dl select_recommend_id="'+i+'"><dt><h4 class="dialog-handle-title">商品推荐模块标题名称</h4>'+
    			...
    			'<span class="right-country-detail">修改该区域中部推荐商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏</span>'+
...
function update_recommend() {//更新
	...
	if (get_text=='1') {
		...
		$(".right-country-detail dl[recommend_id='"+id+"'] dt h4").html(recommend_name);
	  ...
	  $(".right-country-detail dl[recommend_id='"+id+"'] dd").html('<ul class="goods-list-country">'+text_append+'</ul>');
