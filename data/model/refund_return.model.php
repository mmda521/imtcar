 ...
 *
 *
 * by 33hao 好商城V3  www.shopnc.net 开发
 */
...
class refund_returnModel extends Model{
		...
		if ($refund_id > 0) {
    	    ...
    	    try {
    	        ...
    	        if ($predeposit_amount > 0) {//退预存款
                    ...
                    $log_array['amount'] = 0 ;//jinp0808退预存款金额
    ...
    public function editOrderRefund_jp($refund) {
        ...
        if ($refund_id > 0) {
            Language::read('model_lang_index');
            $order_id = $refund['order_id'];//订单编号
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,pd_amount';//mx 多获取预存款支付金额一列
            $model_order = Model('order');
            $order = $model_order->getOrderInfo(array('order_id'=> $order_id),array('order_common'),$field);
            ...
            try {
                ...
                $api_amount = $predeposit_amount-$order['pd_amount'];
