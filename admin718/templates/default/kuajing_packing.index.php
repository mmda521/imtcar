...
<div class="page">
      ...
      <tbody>
        <tr>
          ...
          <td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query']; ?>">&nbsp;</a>
            ...
            <a class="btns " href="index.php?act=kuajing_packing&op=index" title="<?php echo $lang['cancel_search'];?>"><span>取消检索<?php echo $lang['cancel_search'];?></span></a>
            <?php }?></td>
        </tr>
      </tbody>
    </table>
  ...
  <form id="form_grade" method='post' name="">
    <input type="hidden" name="packing_id" id="packing_id" value="ok" />
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
        <th class="w24">&nbsp;</th>
          <th class="align-center">序号</th>
          <th class="align-center">包装方式名称</th>
      ...
      <tbody>
        ...
        <?php $no++;?>
        <tr class="hover">
        <td><input type="checkbox" name='check_id[]' value="<?php echo $v['packing_id'];?>" class="checkitem"></td>
          <td class="align-center"><?php echo $no;?></td>
          <td class="align-center"><?php echo $v['packing_name'];?></td>
          <td class="align-center"><?php echo $v['code_guan'];?></td>
          <td class="align-center"><?php echo $v['code_jian'];?></td>
          <td class="w270 align-center">
		    <a href="index.php?act=kuajing_packing&op=edit&packing_id=<?php echo $v['packing_id'];?> "><?php echo $lang['nc_edit'];?></a> |<a href="javascript:submit_delete(<?php echo $v['packing_id'];?>)">
...
function submit_delete(packing_id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        $('#form_grade').attr('method','post');
        $('#form_grade').attr('action','index.php?act=kuajing_packing&op=del');
        $('#packing_id').val(packing_id);
        $('#form_grade').submit();
    }
}
</script>
