...
<div class="page">
  <table class="table tb-type2 order">
    <tbody>
      ...
      <tr>
        <td colspan="2"><ul>
            ...
            <li><strong><?php echo $lang['order_state'];?>:</strong><?php if($output['order_info']['refund_state']=='1'){echo '部分退款';}else if($output['order_info']['refund_state']=='2'){echo '已关闭';}else{echo orderState($output['order_info']);} ?></li>
    ...
    <?php if(is_array($output['order_log']) and !empty($output['order_log'])) { ?>
      ...
      <tr>
        <td>
          <?php echo $val['log_role']; ?> <?php echo $val['log_user']; ?>&emsp;<?php echo $lang['order_show_at'];?>&emsp;<?php echo date("Y-m-d H:i:s",$val['log_time']); ?>&emsp;
            <?php if($val['log_msg']=='取消了订单 ( 商品全部退款完成取消订单 )'){if($output['order_info']['refund_state']=='1'){
                echo '取消了订单 ( 商品部分退款完成取消订单 )';
            }else{
                echo $val['log_msg'];
            }}else{echo $val['log_msg'];}  ?>
        </td>
      </tr>
      <?php } ?>
    <?php } ?>
