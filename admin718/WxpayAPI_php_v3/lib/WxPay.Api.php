...
class WxPayApi
{
	 ...
	 * jinp_0825_customs
	 *
	 * 支付报关，WxPayCustoms中out_trade_no、transaction_id必须
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayCustoms $inputObj
	 * @param int $timeOut
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */
	public static function customs($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/cgi-bin/mch/customs/customdeclareorder";
		//$url = "http://api.mch.tenpay.com/cgi-bin/mch_custom_declare.cgi";
		...
		//if(!$inputObj->IsOut_trade_noSet()) {
		//	throw new WxPayException("缺少支付报关接口必填参数out_trade_no！");
		//}else if(!$inputObj->IsTransaction_idSet()){
		//	throw new WxPayException("缺少支付报关接口必填参数transaction_id！");
		//}
		...
		if(!$inputObj->IsTransaction_idSet()) {
			...
			throw new WxPayException("why 缺少支付报关接口必填参数out_trade_no！");
		}
		...
		//微信支付报关jinp_0826
		$inputObj->SetCustoms(WxPayConfig::CUSTOMS);//海关备案号
		$inputObj->SetMch_customs_no(WxPayConfig::MCH_CUSTOMS_NO);//商户海关备案号

		//print_r($inputObj->SetCustoms(WxPayConfig::CUSTOMS));
		...
		echo "</br>";
		//$wxpay = $xml->getElementsByTagName( "xml" )->item(0)->nodeValue;
		//echo $wxpay;
		echo "</br>";
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		print_r($xml);
		echo "</br>";
		$wxpay = $doc->getElementsByTagName( "xml" )->item(0)->nodeValue;
		echo $wxpay;
		//die;
