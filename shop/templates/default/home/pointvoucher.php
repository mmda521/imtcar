  ...
  <div class="ncp-main-layout">
    ...
    <ul class="ncp-voucher-list">
      <?php foreach ($output['voucherlist'] as $k=>$v){?>
          <?php if($v['voucher_t_display']==0 || ($v['voucher_t_display']==1)){?>
            <li>
        <div class="ncp-voucher">
          ...
          <div class="info"><a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$v['voucher_t_store_id']));?>" class="store"><?php echo $v['voucher_t_storename'];?></a>
            <p class="store-classify"><?php echo $v['voucher_t_gc_name'];?></p>
            ...
            <dl class="value">
                ...
                <?php $k=intval($v['voucher_t_validity'])-1;?>
                <?php $d=strtotime("+$k day");?>
                <?php if($v['voucher_t_validity']&&($d<=$v['voucher_t_end_date'])) {?>
                    <dd class="time">有效期至<?php echo @date('Y-m-d',strtotime("+$k day"));?></dd>
                <?php }else{ ?>
                    <?php $end=$v['voucher_t_end_date']-3600*24*1;?>
                    <dd class="time">有效期至<?php echo @date('Y-m-d',$end);?></dd>
                <?php }?>
            </dl>
          <div class="point">
