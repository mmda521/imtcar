  ...
  <form method="post" action="index.php?act=pointvoucher&op=voucherexchange_save" id="exform" onsubmit="ajaxpost('exform', '', '', 'onerror');">
    ...
    <dl>
      <dt>您正在使用<span class="ml5 mr5"><?php echo $output['template_info']['voucher_t_points'];?></span><?php echo$lang['points_unit'];?>&nbsp;兑换&nbsp;1&nbsp;张<br/>
        <?php echo $output['template_info']['store_name'];?><?php echo $output['template_info']['voucher_t_price'].$lang['currency_zh'];?>店铺代金券（<em>
          <em><?php foreach($output['gc_list'] as $v =>$gc) {?>
              <?php if($output['template_info']['voucher_t_gc_id']==$v) {?>
                <?php echo $gc['gc_name'];?>
              <?php }?>
            <?php }?>满<?php echo $output['template_info']['voucher_t_limit'];?>减<?php echo $output['template_info']['voucher_t_price'];?></em>）</dt>
      <?php $k=intval($output['template_info']['voucher_t_validity'])-1;?>
      <?php $d=strtotime("+$k day");?>
      <?php if($output['template_info']['voucher_t_validity']&&($d<=$output['template_info']['voucher_t_end_date'])) {?>
        <dd>店铺代金券有效期至<?php echo @date('Y-m-d',strtotime("+$k day"));?></dd>
      <?php }else{ ?>
        <?php $end=$output['template_info']['voucher_t_end_date']-3600*24*1;?>
        <dd>店铺代金券有效期至<?php echo @date('Y-m-d',$end);?></dd>
      <?php }?>
      <dd>
