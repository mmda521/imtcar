  ...
  <ul class="list_pic">
    ...
    <li class="item">
      <div class="goods-content" nctype_goods=" <?php echo $value['goods_id'];?>" nctype_store="<?php echo $value['store_id'];?>">
        ...
        <?php }?>
        <?php if ($value['is_mode'] == 1) {?>
        <div class="goods-promotion-beihuo"><span></span></div>
        <?php } elseif ($value['is_mode'] == 2) {?>
        <div class="goods-promotion-jihuo"><span></span></div>
        <?php }?>
        <div class="goods-info">
          ...
          <div class="goods-price"> <em class="sale-price" title="<?php echo $lang['goods_class_index_store_goods_price'].$lang['nc_colon'].$lang['currency'].$value['goods_promotion_price'];?>"><?php echo ncPriceFormatForList($value['goods_promotion_price']);?></em> 
            <?php if($value['goods_marketprice']!= 0) {?>
          ...
          <div class="goods-sub">
            ...
            <?php 
            	...
            	$is_own_shop = $model_store->getfby_store_id($value['store_id'],'is_own_shop');

            if($is_own_shop == 1){?>
            <div class="self-support"> <span title="自营">郑欧班列自营</span></div>
            <?php }?>
          ...
          <div class="sell-stat">
            <ul>
              <li><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $value['goods_id']));?>#ncGoodsRate" target="_blank" class="status"><?php echo $value['goods_presalenum']+$value['goods_salenum'];?></a>
                <p>商品销量</p>
              </li>
