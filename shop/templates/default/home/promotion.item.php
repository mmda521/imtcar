...
<div class="item">
  <div class="scope">
    ...
    <?php if($goods_info['buyNow']=='1') {?>
    <div class="goods-buy"><a href="<?php if ($_SESSION['is_login']) {?>javascript:void(0);<?php } else {?>javascript:login_dialog();<?php }?>" nctype="add_cart" data-param="{goods_id:<?php echo $goods_info['goods_id'];?>}" class="btn_xianshi1">立即抢购</a> <span class="raty" data-score="<?php echo $goods_info['evaluation_good_star'];?>" style="width: 100px;"></span> <span class="mt5"><a href="<?php echo urlShop('show_store','index',array('store_id'=>$goods_info['store_id']));?>"><?php echo $goods_info['store_name'];?></a></span> </div>
    <?php } else {?>
    <div class="goods-buy"><a  nctype="" data-param="{goods_id:<?php echo $goods_info['goods_id'];?>}" class="btn_xianshi2">即将开始</a> <span class="raty" data-score="<?php echo $goods_info['evaluation_good_star'];?>" style="width: 100px;"></span> <span class="mt5"><a href="<?php echo urlShop('show_store','index',array('store_id'=>$goods_info['store_id']));?>"><?php echo $goods_info['store_name'];?></a></span> </div>
    <?php }?>
    <!--  
    <ul class="goodseval">
