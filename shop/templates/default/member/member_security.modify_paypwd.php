  ...
  <div class="ncm-default-form">
    <form method="post" id="password_form" name="password_form" action="index.php?act=member_security&op=modify_paypwd">
      ...
      <dl>
        ...
        <dd>
          <input type="password"  maxlength="6" class="password" name="password" id="password" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)"  onpaste="return false;"/>
          <label for="password" generated="true" class="error"></label>
          <p class="hint">6位数字组成。</p></dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>确认支付密码：</dt>
        <dd>
          <input type="password" maxlength="6" class="password" name="confirm_password" id="confirm_password"  onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onpaste="return false;"/>
    ...
    $('#password_form').validate({
        ...
        messages : {
            password  : {
                ...
                minlength : '<i class="icon-exclamation-sign"></i>密码不能少于6位'
            },
            confirm_password : {
                required   : '<i class="icon-exclamation-sign"></i>请正确输入密码',
