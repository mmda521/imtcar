...
class goodsControl extends BaseGoodsControl {
	...
	Tpl::output('mansong_all', $goods_detail['mansong_all']);//xinjia
		...
		$goods_common = $model_goods->searchforcommonid($goods_id);
		
		//车辆基本参数	
		 $goodsbasic_info = $model_goods->getbasicInfo($goods_common['goods_commonid']);
		 Tpl::output('basic', $goodsbasic_info);
		 //车身参数
		 $goodscarbody_info = $model_goods->getcarbodyInfo($goods_common['goods_commonid']);
		 Tpl::output('carbody', $goodscarbody_info);
		 //汽车发动机参数
		 $goodsengine_info = $model_goods->getengineInfo($goods_common['goods_commonid']);
		 Tpl::output('engine', $goodsengine_info);
		 //变速箱基本参数
        $goodsgearbox_info = $model_goods->getgearboxInfo($goods_common['goods_commonid']);
        Tpl::output('gearbox', $goodsgearbox_info);
        //底盘转向基本参数
        $goodssteer_info = $model_goods->getsteerInfo($goods_common['goods_commonid']);
        Tpl::output('steer', $goodssteer_info);
        //车轮制动基本参数
        $goodswheel_info = $model_goods->getwheelInfo($goods_common['goods_commonid']);
        Tpl::output('wheel', $goodswheel_info);
        //主/被动安全装备基本参数
        $goodssafe_items_info = $model_goods->getsafe_itemsInfo($goods_common['goods_commonid']);
        Tpl::output('safe_items', $goodssafe_items_info);
        //座椅配置基本参数
        $goodsseat_config_info = $model_goods->getseat_configInfo($goods_common['goods_commonid']);
        Tpl::output('seat_config', $goodsseat_config_info);
        //多媒体配置基本参数
        $goodswmm_config_info = $model_goods->getwmm_configInfo($goods_common['goods_commonid']);
        Tpl::output('wmm_config', $goodswmm_config_info);
		 //内部配置基本参数
        $goodsinner_config_info = $model_goods->getinner_configInfo($goods_common['goods_commonid']);
        Tpl::output('inner_config', $goodsinner_config_info);
        //外部/防盗配置基本参数
        $goodsexternal_config_info = $model_goods->getexternal_configInfo($goods_common['goods_commonid']);
        Tpl::output('external_config', $goodsexternal_config_info);
        //操控配置基本参数
        $goodscontrol_config_info = $model_goods->getcontrol_configInfo($goods_common['goods_commonid']);
        Tpl::output('control_config', $goodscontrol_config_info);
        //灯光配置基本参数
        $goodslight_config_info = $model_goods->getlight_configInfo($goods_common['goods_commonid']);
        Tpl::output('light_config', $goodslight_config_info);
        //玻璃/后视镜基本参数
        $goodsrearview_mirror_info = $model_goods->getrearview_mirrorInfo($goods_common['goods_commonid']);
        Tpl::output('rearview_mirror', $goodsrearview_mirror_info);
        //高科技配置基本参数
        $goodshigh_tech_config_info = $model_goods->gethigh_tech_configInfo($goods_common['goods_commonid']);
        Tpl::output('high_tech_config', $goodshigh_tech_config_info);
        //空调/冰箱基本参数
        $goodsrefrigerator_info= $model_goods->getrefrigeratorInfo($goods_common['goods_commonid']);
