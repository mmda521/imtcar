    ...
    public function export_order_subOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $data = $model_order->getOrderList2($consignee_name,$condition,'','*','order_id desc',self::EXPORT_SIZE,array('order_common','member','goods_kuajing_d'));
            ...
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_order->getOrderList2($consignee_name,$condition,'','*','order_id desc',"{$limit1},{$limit2}",array('order_common','member','goods_kuajing_d'));
